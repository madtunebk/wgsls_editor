...
impl AudioState {
    ...
    pub fn set_bands(&self, bass: f32, mid: f32, high: f32) {
        if let Ok(mut b) = self.bass.lock() { *b = bass.clamp(0.0, 1.0); }
        if let Ok(mut m) = self.mid.lock() { *m = mid.clamp(0.0, 1.0); }
        if let Ok(mut h) = self.high.lock() { *h = high.clamp(0.0, 1.0); }
    }
...
pub fn start_input_fft(audio: Arc<AudioState>) -> Option<cpal::Stream> {
    ...
    let device = match host.default_input_device() {
        Some(d) => d,
        None => { warn!("No default input device"); return None; }
    };
    let config = match device.default_input_config() {
        Ok(c) => c,
        Err(e) => { warn!("No default input config: {e}"); return None; }
    };
        ...
        loop {
            ...
            for i in 0..FFT_LEN {
                ...
                for _c in 0..channels {
                    let v = match rx.recv() { Ok(v) => v, Err(_) => return }; // stream ended
                    acc += v;
                }
            ...
            let mut b = if bass_n > 0 { bass_e / bass_n as f32 } else { 0.0 };
            let mut m = if mid_n > 0 { mid_e / mid_n as f32 } else { 0.0 };
            let mut h = if high_n > 0 { high_e / high_n as f32 } else { 0.0 };

            // Normalize roughly (depends on input gain)
            let norm = 0.1f32; // empirical scale
            b *= norm; m *= norm; h *= norm;
            b = b.clamp(0.0, 1.0);
            m = m.clamp(0.0, 1.0);
            h = h.clamp(0.0, 1.0);

            // Smooth
            sb = if b > sb { sb + attack * (b - sb) } else { sb + decay * (b - sb) };
            sm = if m > sm { sm + attack * (m - sm) } else { sm + decay * (m - sm) };
            sh = if h > sh { sh + attack * (h - sh) } else { sh + decay * (h - sh) };
    ...
    let stream = match config.sample_format() {
        cpal::SampleFormat::F32 => {
            build_input_stream_f32(&device, &cfg, tx)
        }
        cpal::SampleFormat::I16 => {
            build_input_stream_i16(&device, &cfg, tx)
        }
        cpal::SampleFormat::U16 => {
            build_input_stream_u16(&device, &cfg, tx)
        }
        _ => { warn!("Unsupported sample format"); return None; }
    }.ok()?;

    if let Err(e) = stream.play() { warn!("Failed to start input stream: {e}"); return None; }
    Some(stream)
}

fn build_input_stream_f32(device: &cpal::Device, config: &cpal::StreamConfig, tx: Sender<f32>) -> Result<cpal::Stream, cpal::BuildStreamError> {
    ...
    device.build_input_stream(
        ...
        move |data: &[f32], _| {
            for frame in data.chunks_exact(channels) {
                for &s in frame { let _ = tx.send(s); }
            }
        },
...
fn build_input_stream_i16(device: &cpal::Device, config: &cpal::StreamConfig, tx: Sender<f32>) -> Result<cpal::Stream, cpal::BuildStreamError> {
    use cpal::traits::DeviceTrait;
    let channels = config.channels as usize;
    device.build_input_stream(
        config,
        move |data: &[i16], _| {
            for frame in data.chunks_exact(channels) {
                for &s in frame { let _ = tx.send(s as f32 / i16::MAX as f32); }
            }
        },
        move |err| warn!("Audio input error: {err}"),
        None,
...
fn build_input_stream_u16(device: &cpal::Device, config: &cpal::StreamConfig, tx: Sender<f32>) -> Result<cpal::Stream, cpal::BuildStreamError> {
    use cpal::traits::DeviceTrait;
    let channels = config.channels as usize;
    device.build_input_stream(
        config,
        move |data: &[u16], _| {
            for frame in data.chunks_exact(channels) {
                for &s in frame { let _ = tx.send((s as f32 / u16::MAX as f32) * 2.0 - 1.0); }
            }
        },
        move |err| warn!("Audio input error: {err}"),
        None,
